...
public class MultiplySet extends ReferenceExpressionOperator {
	
	public MultiplySet(Reference r, Expression e, SourceInfo uc) {
		super(r, e, uc);
	}

	protected SValue getUpdateValue(SValue originalValue, SValue expressionValue) throws ScriptExecutionException {
		try {
			return Operations.multiply(originalValue, expressionValue);
