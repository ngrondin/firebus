	...
	public SValue call(SValue... arguments) throws ScriptException {
		if(arguments.length > 0) {
			...
			if(v instanceof SNumber) {
				...
				ZonedDateTime nzdt = ZonedDateTime.of(zdt.getYear(), zdt.getMonth().getValue(), i, zdt.getHour(), zdt.getMinute(), zdt.getSecond(), zdt.getNano(), zdt.getZone());
				date.setZonedDateTime(nzdt);
				return new SNumber(nzdt.toInstant().toEpochMilli());
			} else {
				throw new ScriptException("setDate requires a number");
			}
		} else {
			throw new ScriptException("setDate requires at lease 1 argument");
		}
	}
